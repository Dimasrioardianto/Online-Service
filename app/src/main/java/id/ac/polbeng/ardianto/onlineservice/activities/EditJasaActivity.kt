package id.ac.polbeng.ardianto.onlineservice.activities
...
import id.ac.polbeng.ardianto.onlineservice.databinding.ActivityEditJasaBinding
import id.ac.polbeng.ardianto.onlineservice.helpers.Config
import id.ac.polbeng.ardianto.onlineservice.helpers.Config.Companion.EXTRA_JASA
        ...
        binding.btnEditJasa.setOnClickListener {
            ...
            requestCall.enqueue(object : retrofit2.Callback<DefaultResponse> {
                override fun onFailure(call: Call<DefaultResponse>, t: Throwable) {
                    showLoading(false)
                    Toast.makeText(this@EditJasaActivity, "Error terjadi: ${t.message}", Toast.LENGTH_LONG).show()
                }

                override fun onResponse(call: Call<DefaultResponse>, response: Response<DefaultResponse>) {
                    showLoading(false)
                    if (response.body()?.error == false) {
                        Toast.makeText(this@EditJasaActivity, response.body()?.message, Toast.LENGTH_LONG).show()
                        finish()
                    } else {
                        Toast.makeText(this@EditJasaActivity, "Gagal: ${response.body()?.message}", Toast.LENGTH_LONG).show()
                    }
                }
            })
        }
                ...
                requestCall.enqueue(object : retrofit2.Callback<DefaultResponse> {
                    ...
                    override fun onResponse(call: Call<DefaultResponse>, response: Response<DefaultResponse>) {
                        if (!response.body()?.error!!) {
                            ...
                            defaultResponse.let {
                                Toast.makeText(this@EditJasaActivity, defaultResponse.message, Toast.LENGTH_LONG).show()
                                finish()
                            }
                        } else {
                            Toast.makeText(
